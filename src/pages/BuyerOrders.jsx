...
import { Package, Loader2, CheckCircle2, ShieldCheck, Zap } from "lucide-react";
import api from "../api/axios";
import toast from "react-hot-toast";

const StatusBadge = ({ status }) => {
  const styles = {
    paid: "bg-blue-600 text-white border-2 border-blue-800",
    completed: "bg-green-600 text-white border-2 border-green-800",
...
function BuyerOrders() {
  ...
  const fetchOrders = useCallback(async (isSilent = false) => {
    try {
      ...
      if (!isSilent) toast.error("Sync failed. Check your connection.");
    } finally {
      setLoading(false);
      setIsSyncing(false);
    }
  ...
  // AUTOMATION: 2026 Callback Sync logic
  // Since orders are created via callback, we poll every 4 seconds to catch
  // the moment Safaricom hits our backend.
  useEffect(() => {
  ...
  const handleConfirmDelivery = async (orderId) => {
    ...
    try {
      // Hits the route we confirmed: /api/orders/confirm-receipt/<id>
      await api.post(`/orders/confirm-receipt/${orderId}`);

      toast.success("FUNDS RELEASED: Farmer has been paid.", {
        id: loadToast,
      });
      fetchOrders();
    } catch (err) {
  ...
  const getAnimalName = (items) => {
    ...
    // Backend returns JSON, so we handle both object and string formats
      ...
      <div className="flex justify-between items-end border-l-[12px] border-green-600 pl-8 py-4 bg-slate-50 shadow-[8px_8px_0px_0px_rgba(0,0,0,0.1)]">
        <div>
          ...
          <p className="text-slate-500 font-bold mt-3 uppercase text-[10px] tracking-[0.3em] flex items-center gap-2">
            <ShieldCheck size={14} className="text-green-600" /> Secure M-Pesa
            Escrow Interface • 2026
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-6">
          {orders.map((order) => (
            <div
              key={order.id}
              className="group bg-white border-4 border-black p-8 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] transition-transform hover:-translate-y-1">
                ...
                <div className="border-t-2 lg:border-t-0 lg:border-l-4 border-slate-100 pt-6 lg:pt-0 lg:pl-10">
                  {order.status === "paid" ? (
                    ...
                    <div className="flex flex-col items-center lg:items-end gap-2">
                      <div className="flex items-center gap-2 py-3 px-5 bg-slate-50 border-2 border-green-500 text-green-700">
